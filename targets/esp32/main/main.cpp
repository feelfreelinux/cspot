...
void init_spiffs() {
  ...
  if (ret != ESP_OK) {
    if (ret == ESP_FAIL) {
      CSPOT_LOG(error, "Failed to mount or format filesystem");
    } else if (ret == ESP_ERR_NOT_FOUND) {
      CSPOT_LOG(error, "Failed to find SPIFFS partition");
    } else {
      CSPOT_LOG(error, "Failed to initialize SPIFFS (%s)",
                esp_err_to_name(ret));
    }
    return;
  }
  ...
  if (ret != ESP_OK) {
    CSPOT_LOG(error, "Failed to get SPIFFS partition information (%s)",
              esp_err_to_name(ret));
  } else {
...
void app_main(void) {
  ...
  CSPOT_LOG(info, "Connected to AP, start spotify receiver");
