...
`cspot` is meant to be used as a lightweight C++ library for playing back Spotify music and recieve control notifications from Spotify connect. 
...
Addidtionally the following audio sinks are implemented for the esp32 target:
- [`ES9018AudioSink`](targets/esp32/main/sinks/ES9018AudioSink.cpp) - provides playback via the ES9018 DAC connected to the ESP32
- [`I2SAudioSink`](targets/esp32/main/sinks/I2SAudioSink.cpp) - enables I2S output from the ESP32. This AudioSink allows hardware volume control.
- TODO: internal esp32 DAC for crappy quality testing.

Each new audio sink must implement the `void feedPCMFrames(std::vector<uint8_t> &data)` method which should accept stereo PCM audio data at 44100 Hz and 16 bits per sample. Please note that the sink should somehow buffer the data, because playing it back may result in choppy audio.
