...
namespace cspot {
  ...
  public:
    ...
    uint64_t longestInterval = 0, totalAmountOfPlayTime = 0, totalMsOfPlayedTrack = 0,
      totalAmountPlayed = 0, audioKeyTime = 0, trackHeaderTime = 0,
      written_bytes = 0, track_size = 0, timestamp = 0;
  ...
  private:
    ...
    std::vector<std::pair<uint64_t, uint64_t>> addInterval(std::vector<std::pair<uint64_t, uint64_t>>& intervals, std::pair<std::uint64_t, std::uint64_t> newInterval) {
  ...
  public:
    ...
    PlaybackMetrics(std::shared_ptr<cspot::Context> ctx) {
      this->ctx = ctx;
    }
    enum reason {
      TRACK_DONE,
      ...
      REMOTE,
      UNKNOWN
    }end_reason = UNKNOWN, start_reason = UNKNOWN, nested_reason = UNKNOWN;
